...
local message
message="$1"
echo "uh-oh something went wrong at [$timestamp] for invalid option [$message] ; Refer to help section (sysconfig.sh -h)"
}
...
function error-exit {
echo "some text" | logger -t "$(basename "$0")" -i -p user.warning
exit 1
...
cat << EOF
	...
	sysconfig.sh Help Page
      --------------------------
      Usage : sysconfig.sh [OPTION]
      Valid Arguments:
           -h (Display help page)
           --host (Display hostname info)
           --domain (Display domain info)
           --ipconfig (Display IP Config info)
           --os (Display Operating System info)
